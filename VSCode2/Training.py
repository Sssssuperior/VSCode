...
from Models.ImageDepthNet_t import ImageDepthNet_t
import os
...
def main(local_rank, num_gpus, args):
    ...
    net_s = ImageDepthNet(args)
    net_t = ImageDepthNet_t(args)
    ...
    net_s = nn.SyncBatchNorm.convert_sync_batchnorm(net_s)
    net_s = torch.nn.parallel.DistributedDataParallel(
        net_s,
        device_ids=[local_rank],
        output_device=local_rank,
        find_unused_parameters=True)
    net_t = nn.SyncBatchNorm.convert_sync_batchnorm(net_t)
    net_t = torch.nn.parallel.DistributedDataParallel(
        net_t,
        device_ids=[local_rank],
        output_device=local_rank,
        find_unused_parameters=True)

    model_path_s = './noprompt.pth' ### replace for no prompt verision in stage 1
    model_path_t = './RGB_VST_S.pth' ### replace for conference version
    state_dict_s = torch.load(model_path_s)
    state_dict_t = torch.load(model_path_t)
    ...
    print('Student model loaded from {}'.format(model_path_s))
    net_t.load_state_dict(new_state_dict1, strict=False)
    print('Teacher model loaded from {}'.format(model_path_t))

    for name, params in net_s.named_parameters():
        if (name not in previous_parameter):
            print(name)

    base_params = [params for name, params in net_s.named_parameters() if ('backbone' in name)]
    other_params = [params for name, params in net_s.named_parameters() if ('backbone' not in name)]
    ...
    train_loaderR = torch.utils.data.DataLoader(train_datasetR, batch_size=args.batch_size // 5, num_workers=6,
                                                pin_memory=True,
                                                sampler=samplerR,
                                                drop_last=True,
                                                )
    train_loaderD = torch.utils.data.DataLoader(train_datasetD, batch_size=args.batch_size // 5, num_workers=6,
                                               pin_memory=True,
                                               sampler=samplerD,
                                               drop_last=True,
                                               )
    train_loaderT = torch.utils.data.DataLoader(train_datasetT, batch_size=args.batch_size // 5, num_workers=6,
                                                pin_memory=True,
                                                sampler=samplerT,
                                                drop_last=True,
                                                )
    train_loaderV = torch.utils.data.DataLoader(train_datasetV, batch_size=args.batch_size // 5, num_workers=6,
                                                pin_memory=True,
                                                sampler=samplerV,
                                                drop_last=True,
                                                )
    train_loaderCR = torch.utils.data.DataLoader(train_datasetCR, batch_size=args.batch_size // 5, num_workers=6,
                                                pin_memory=True,
                                                sampler=samplerCR,
                                                drop_last=True,
                                                )
    train_loaderCV = torch.utils.data.DataLoader(train_datasetCV, batch_size=args.batch_size // 5, num_workers=6,
                                                 pin_memory=True,
                                                 sampler=samplerCV,
                                                 drop_last=True,
                                                 )
    ...
    print(train_loaderCR_length)
    print(train_loaderCV_length)
    batch_size_div = args.batch_size // 5
    ...
    for epoch in range(args.epochs):
        ...
        for epoch in range(args.epochs):
            ...
            for i, data_batchV in enumerate(train_loaderV):
                ...
                with torch.no_grad():
                    outputs_saliency_t, outputs_contour_t, outputs_saliency_s_t, outputs_contour_s_t, domain_prompt_total_t, task_prompt_total2_t, task_prompt_total1_t = net_t(images, depths)
                outputs_saliency, outputs_contour, outputs_saliency_s, outputs_contour_s, domain_prompt_total, task_prompt_total2, task_prompt_total1, fea_fg, fea_bg, prompt_aggregated = net_s(images, depths)
                ...
                mask_1_16_t, mask_1_8_t, mask_1_4_t, mask_1_1_t = outputs_saliency_t
                cont_1_16_t, cont_1_8_t, cont_1_4_t, cont_1_1_t = outputs_contour_t
                mask_1_16_s_t, mask_1_8_s_t, mask_1_4_s_t, mask_1_1_s_t = outputs_saliency_s_t
                cont_1_16_s_t, cont_1_8_s_t, cont_1_4_s_t, cont_1_1_s_t = outputs_contour_s_t
                ...
                # contrastive loss
                # define tempreature = 1
                # add normalization
                real_prompt = F.normalize(prompt_aggregated, dim=2)
                fea_t = F.normalize(fea_fg, dim=2)
                positive_sim = torch.exp(torch.matmul(real_prompt, fea_t.permute(0, 2, 1)))
                fea_n = F.normalize(fea_bg, dim=2)
                all_sim = positive_sim + torch.exp(torch.matmul(real_prompt, fea_n.permute(0, 2, 1)))
                eps = 1e-10
                contrastive_loss = (-torch.log(positive_sim / (all_sim + eps))).sum(dim=0).squeeze(-1).squeeze(-1)

                # # distiliation loss
                loss5_t = criterion(mask_1_16, sigmoid(mask_1_16_t))
                loss4_t = criterion(mask_1_8, sigmoid(mask_1_8_t))
                loss3_t = criterion(mask_1_4, sigmoid(mask_1_4_t))
                loss1_t = criterion(mask_1_1, sigmoid(mask_1_1_t))

                # contour loss
                c_loss5_t = criterion(cont_1_16, sigmoid(cont_1_16_t))
                c_loss4_t = criterion(cont_1_8, sigmoid(cont_1_8_t))
                c_loss3_t = criterion(cont_1_4, sigmoid(cont_1_4_t))
                c_loss1_t = criterion(cont_1_1, sigmoid(cont_1_1_t))

                loss5_s_t = criterion(mask_1_16_s, sigmoid(mask_1_16_s_t))
                loss4_s_t = criterion(mask_1_8_s, sigmoid(mask_1_8_s_t))
                loss3_s_t = criterion(mask_1_4_s, sigmoid(mask_1_4_s_t))
                loss1_s_t = criterion(mask_1_1_s, sigmoid(mask_1_1_s_t))

                # contour loss
                c_loss5_s_t = criterion(cont_1_16_s, sigmoid(cont_1_16_s_t))
                c_loss4_s_t = criterion(cont_1_8_s, sigmoid(cont_1_8_s_t))
                c_loss3_s_t = criterion(cont_1_4_s, sigmoid(cont_1_4_s_t))
                c_loss1_s_t = criterion(cont_1_1_s, sigmoid(cont_1_1_s_t))
                ...
                # distill loss
                img_total_loss_t = loss_weights[0] * loss1_t + loss_weights[2] * loss3_t + loss_weights[3] * loss4_t + loss_weights[4] * loss5_t
                contour_total_loss_t = loss_weights[0] * c_loss1_t + loss_weights[2] * c_loss3_t + loss_weights[3] * c_loss4_t + loss_weights[4] * c_loss5_t
                img_total_loss_s_t = loss_weights[0] * loss1_s_t  + loss_weights[2] * loss3_s_t + loss_weights[3] * loss4_s_t + loss_weights[4] * loss5_s_t
                contour_total_loss_s_t = loss_weights[0] * c_loss1_s_t + loss_weights[2] * c_loss3_s_t+ loss_weights[3] * c_loss4_s_t + loss_weights[4] * c_loss5_s_t

                img_total_loss_i = loss_weights[0] * loss1_i
                img_total_loss_s_i = loss_weights[0] * loss1_s_i

                total_loss = img_total_loss + contour_total_loss + img_total_loss_s + contour_total_loss_s + img_total_loss_i + img_total_loss_s_i + cosin_loss + (img_total_loss_t + contour_total_loss_t + img_total_loss_s_t + contour_total_loss_s_t) + contrastive_loss
                ...
                if (i + 1) % accum_steps == 0 or (i + 1) == len(train_loaderV):
                    ...
                    if (local_rank == 0) and (whole_iter_num == args.train_steps):
                        torch.save(net_s.state_dict(), args.save_model_dir + 'RGB_VST.pth')
            ...
            torch.save(net_s.state_dict(), args.save_model_dir + 'RGB_VST_itter.pth')