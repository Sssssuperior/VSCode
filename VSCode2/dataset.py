...
def load_list(dataset_list, data_root, task):
    ...
    contours = []
    tasks = []
    change_root = "./Data/pseudo_new/" ### change for pseudo dataset path
    dataset_list = dataset_list.split('+')

    if task == "RGB":
        for dataset_name in dataset_list:
            if dataset_name == "DUTS/DUTS-TR":
                img_root1 = data_root + dataset_name + '/DUTS-TR-Image/'
                img_files1 = os.listdir(img_root1)

                for img in img_files1:
                    images.append(img_root1 + img[:-4] + '.jpg')
                    labels.append(img_root1.replace('/DUTS-TR-Image/', '/DUTS-TR-Mask/') + img[:-4] + '.png')
                    contours.append(img_root1.replace('/DUTS-TR-Image/', '/DUTS-TR-Contour/') + img[:-4] + '.png')
            else:
                num = len(images)
                img_root = change_root + dataset_name + '/RGB/'
                img_files2= os.listdir(img_root)
                if num > len(img_files2):
                    num = len(img_files2)
                    img_files = img_files2
                else:
                    img_files = random.sample(img_files2, num)
        ...
        while len(images) < 18000:
            img = random.choice(img_files1)
            images.append(img_root1 + img[:-4] + '.jpg')
            labels.append(img_root1.replace('/DUTS-TR-Image/', '/DUTS-TR-Mask/') + img[:-4] + '.png')
            contours.append(img_root1.replace('/DUTS-TR-Image/', '/DUTS-TR-Contour/') + img[:-4] + '.png')
        ...
        file_all = []
        for dataset_name in dataset_list:
            if "_new_pseudo" in dataset_name:
                img_root = change_root + dataset_name + '/RGB/'
                img_files1 = os.listdir(img_root)
                num = len(images)
                if num > len(img_files1):
                    img_files = img_files1
                else:
                    img_files = random.sample(img_files1, num)

                for img in img_files:
                    images.append(img_root + img[:-4] + '.jpg')
                    labels.append(img_root.replace('/RGB/', '/GT/') + img[:-4] + '.png')
                    depths.append(img_root.replace('/RGB/', '/Depth/') + img[:-4] + '.png')
                    contours.append(img_root.replace('/RGB/', '/Contour/') + img[:-4] + '.png')
            else:
                ...
                file_all += depth_files
            
        while len(images) < 18000:
            depth = random.choice(file_all)
            if depth[-8:] == "left.bmp":
                dataset_name = "NJUD"
            elif depth[-4:] == ".bmp":
                dataset_name = "NLPR"
            else:
                dataset_name = "DUTLF-Depth"
            depth_root = data_root + dataset_name + '/trainset/depth/'
            images.append(depth_root.replace('/depth/', '/RGB/') + depth[:-4] + '.jpg')
            depths.append(depth_root + depth)
            labels.append(depth_root.replace('/depth/', '/GT/') + depth[:-4] + '.png')
        ...
        while len(images) < 18000:
            depth = random.choice(depth_files)
            images.append(depth_root.replace('/T/', '/RGB/') + depth[:-4] + '.jpg')
            depths.append(depth_root + depth)
            labels.append(depth_root.replace('/T/', '/GT/') + depth[:-4] + '.png')
            contours.append(depth_root.replace('/T/', '/Contour/') + depth[:-4] + '.png')
        ...
        for dataset_name in dataset_list:
            if "_new_pseudo" in dataset_name:
                ...
                if num > len(img_files):
                    img_files3 = img_files
                else:
                    img_files3 = random.sample(img_files, num)

                for img in img_files3:
                    images.append(img_root + img[:-4] + '.jpg')
                    labels.append(img_root.replace('/RGB/', '/GT/') + img[:-4] + '.png')
                    contours.append(img_root.replace('/RGB/', '/Contour/') + img[:-4] + '.png')
        ...
        while len(images) < 18000:
            ...
            if img[:6] == "COD10K":
                img_root = data_root + "COD10K" + '/GT/'
                labels.append(img_root + img[:-4] + '.png')
                images.append(img_root.replace('/GT/', '/RGB/') + img[:-4] + '.jpg')
                contours.append(img_root.replace('/GT/', '/Contour/') + img[:-4] + '.png')
            else:
                img_root = data_root + "CAMO" + '/RGB/'
                images.append(img_root + img[:-4] + '.jpg')
                labels.append(img_root.replace('/RGB/', '/GT/') + img[:-4] + '.png')
                contours.append(img_root.replace('/RGB/', '/Contour/') + img[:-4] + '.png')
        ...
        while len(images) < 18000:
            ...
            depths.append(img[:-4] + '.png')
            images.append(img.replace('/Flow/', '/Imgs/')[:-4]+'.jpg')
            labels.append(img.replace('/Flow/', '/GT/')[:-4] + '.png')
            contours.append(img.replace('/Flow/', '/Contour/')[:-4] + '.png')
        ...
        while len(images) < 18000:
            img = random.choice(all_files)
            if img.split("/")[-4] == "DAVIS":
                depths.append(img.replace('/RGB/', '/Flow/')[:-4] + '.jpg')
            else:
                depths.append(img.replace('/RGB/', '/Flow/')[:-4] + '.png')
            images.append(img)
            labels.append(img.replace('/RGB/', '/GT/')[:-4] + '.png')
            contours.append(img.replace('/RGB/', '/Contour/')[:-4] + '.png')
...
class ImageData(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train':
            ...
            # if self.depth_path[item]==None:
            #     trans_no = transforms.ToTensor()
            #     new_depth = trans_no(Image.new('RGB', (self.img_size, self.img_size), (0, 0, 0)))
...
def get_loader(dataset_list, data_root, img_size, mode='train', task=None):
    ...
    if mode == 'train':
        ...
        dataset = ImageData(dataset_list, data_root, transform_RGB, transformCOD_RGB, transformdepth_RGB,
                            transform_depth, transformtherml_RGB, transform_thermal, transformvideo_RGB,
                            transform_video, mode, task)
