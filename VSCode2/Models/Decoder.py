...
class cross_attn(nn.Module):
    ...
    def forward(self, token, fea):
        B, N, C = fea.shape
        x = self.norm(fea)
        T_s, F_s = token, fea
        # T_s [B, 1, 384]  F_s [B, 14*14, 384]

        q = self.q(T_s).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = self.k(F_s).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(F_s).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        token_infer = (attn @ v).transpose(1, 2).reshape(B, 1, C)
        token_infer = self.proj(token_infer)
        token_infer = self.proj_drop(token_infer)

        token_infer = token + token_infer
        return token_infer
...
class Decoder(nn.Module):
    ...
    def forward(self, fea_1_16, saliency_tokens, contour_tokens, fea_16, saliency_tokens_16, contour_tokens_16, saliency_fea_1_16, contour_fea_1_16, rgb_fea_1_8, rgb_fea_1_4, task_prompt, num):
        ...
        # caculate mask feature
        sigmoid = nn.Sigmoid()
        mask = sigmoid(mask_1_4_s)  # B,1,224*224
        mask = mask.masked_fill(mask < 0.5, float(0.0)).masked_fill(mask >= 0.5, float(1.0)).view(B, 1, -1)
        mask_reverse = mask.masked_fill(mask < 0.5, float(1.0)).masked_fill(mask >= 0.5, float(0.0)).view(B, 1, -1)
        # fea_1_1 B,1,224*224 B,224*224,384 --> B,1,384
        eps = 1e-10
        # foreground feature --> help to judge the task of SOD and COD
        fea_fg = torch.matmul(mask, fea_1_4) / (torch.sum(mask, dim=2) + eps).unsqueeze(2)
        # background feature
        fea_bg = torch.matmul(mask_reverse, fea_1_4) / (torch.sum(mask_reverse, dim=2) + eps).unsqueeze(2)

        return [mask_1_16, mask_1_8, mask_1_4, mask_1_1], [con_1_16, con_1_8, con_1_4, con_1_1],[mask_1_16_s, mask_1_8_s, mask_1_4_s, mask_1_1_s], [con_1_16_s, con_1_8_s, con_1_4_s, con_1_1_s], fea_fg, fea_bg
